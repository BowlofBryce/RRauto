          ...
          <>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Search..."
                className="h-8 pl-8 pr-3 text-sm bg-surface-3 border border-border rounded-md text-text-primary placeholder:text-text-disabled focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent transition-colors w-44"
              />
            </div>
            ...
            <AnimatePresence initial={false}>
              {contacts.map((c, i) => (
                ...
                >
                  ...
                  <span className="text-xs text-text-disabled hidden lg:block w-16 text-right flex-shrink-0">
                    {formatRelative(c.created_at)}
                  </span>
                  <button
                    onClick={() => handleDelete(c.id)}
                    className="opacity-0 group-hover:opacity-100 p-1.5 rounded text-text-disabled hover:text-danger hover:bg-danger-subtle transition-all duration-100"
                  >
                    <Trash2 size={13} />
                  </button>
                </motion.li>
